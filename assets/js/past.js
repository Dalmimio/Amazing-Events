...
let dateCurrent=data.currentDate;//Fecha actual en el js
...
// ----------------------------------------------- F I L T R O  F E C H A ----------------------------------------------

function date(dateEvent, dateTwo){
  return (dateEvent<dateTwo)
}
...
function combinacionDeFiltros() {
  arrayEventFilter = [];
   //por defecto, que muestre todas las cartas anteriores a la fecha actual
  if (arrayChecks.length == 0 && buscadorText == "") {
    arrayEventFilter.push(arrayCartas.filter((evento) =>date(evento.date,dateCurrent)));
   ...
   if (arrayChecks.length > 0 && buscadorText !== "") {
    arrayChecks.forEach((categoria) => {
      arrayEventFilter.push(arrayCartas.filter((evento) =>evento.name.toLowerCase().includes(buscadorText.trim()) && evento.category == categoria && date(evento.date,dateCurrent)));
    });
   ...
   if (arrayChecks.length > 0 && buscadorText == "") {
    ...
    arrayChecks.forEach((categoria) => {
      arrayEventFilter.push(arrayCartas.filter((evento) => evento.category == categoria && date(evento.date,dateCurrent)));
    });

          //si hay algo en el buscador y no en los checks
  } 
   if (arrayChecks.length == 0 && buscadorText !== "") {
    arrayEventFilter.push(arrayCartas.filter((evento) =>evento.name.toLowerCase().includes(buscadorText.trim()) && date(evento.date,dateCurrent)));
  }
    ...
    <div class="card card_tarjeta text-center">
        ...
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between">
            ...
            <a href="../html/describe.html?id=${evento._id}" class="btn_m"><img class="btn_more" src="../img/btn.png" alt="btn"></a>
         
          </div>
        </div>
    </div>`;
      
    contenidoCartaPast.innerHTML = ponerCarta;

    });
  } else {
    contenidoCartaPast.innerHTML = `<div class="container__error"><img src="../img/404 Error with a cute animal-amico.svg" alt="img_error"> <br>
